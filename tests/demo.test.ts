...
test("demo creates a hashed owner login and sample workflows without changing existing workspaces", async () => {
  ...
  try {
    ...
    assert.equal(store.company(orgId).timezone, "Asia/Kolkata");
    assert.equal(
      store.get(orgId, "employees", String(user.employee_id)).email,
      account.email,
    );
    assert.equal(store.list(orgId, "employees").length, 8);
    assert.equal(store.list(orgId, "attendance").length, 40);
    assert.equal(
      store.list(orgId, "leaves").filter((r) => r.status === "Pending").length,
      1,
    );
    assert.equal(
      store.list(orgId, "expenses").filter((r) => r.status === "Approved")
        .length,
      1,
    );
    assert.equal(store.list(orgId, "candidates").length, 4);
    assert.equal(store.list(orgId, "documents").length, 1);
    assert.equal(store.list(orgId, "payroll")[0]?.status, "Draft");
    assert.equal(store.company("existing").name, "Existing company");
    assert.equal(store.list("existing", "employees").length, 0);
