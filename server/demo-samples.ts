...
export async function populateDemoSamples(store: Store) {
  ...
  return await store.transaction(async () => {
    ...
    const owner = actorFrom(ownerRow),
      orgId = owner.orgId;
    const employees = await store.list(orgId, "employees");
    if (
      !(await store.db
    ...
    if (
      ...
      return {
        workspace: DEMO_SLUG,
        roles: (await demoAccounts(store)).map((row) => row.role),
        added: false,
      };
    const baseDate = await today(store, orgId);
    ...
    const add = async (actor: Actor, kind: Kind, body: any, suite = true) =>
      await store.save(
        actor,
        kind,
        suite
    ...
    const employee = await store.get(
      orgId,
      "employees",
      actors.employee.employeeId!,
    );
    for (const actor of [actors.hr, actors.employee]) {
      ...
      await store.save(owner, "documents", {
    ...
    await add(actors.employee, "expenses", {
      employeeId: employee.id,
      spentOn: baseDate,
      amount: 680,
      category: "Travel",
    ...
    await add(actors.employee, "profileRequests", {
      employeeId: employee.id,
      type: "Letter request",
      reason: "Demo request for an employment confirmation letter.",
    });
    await add(owner, "policies", {
    ...
    const nextMonth = new Date(
      ...
      .toISOString()
      .slice(0, 10);
    await add(owner, "employmentChanges", {
      employeeId: employee.id,
      type: "Promotion",
    ...
    if (
      ...
      !(await store.list(orgId, "payroll")).some(
        (period) => period.month === previousMonth,
      )
    ) {
      let period = await createPayroll(store, actors.hr, {
        month: previousMonth,
      });
      for (const line of period.lines)
        period = await editPayroll(store, actors.hr, period, {
