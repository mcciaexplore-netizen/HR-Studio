...
export async function createSession(
  ...
  await store.db
    ...
    .run(digest(token), userId, Date.now() + duration);
  ...
  return async (req, res, next) => {
    try {
      const row = await store.db
        ...
        .get(digest(cookieToken(req.headers.cookie)), Date.now());
      if (!row) return next(new HttpError(401, "Sign in to continue."));
      res.locals.actor = actorFrom(row);
      next();
    } catch (error) {
      next(error);
    }
...
export const requireStaff: RequestHandler = (_req, res, next) =>
  next(
    res.locals.actor.accessRole === "employee"
      ? new HttpError(403, "HR access is required.")
      : undefined,
  );
export const requireOwner: RequestHandler = (_req, res, next) =>
  next(
    res.locals.actor.accessRole !== "owner"
      ? new HttpError(403, "Only the company owner can do this.")
      : undefined,
  );
export function rateLimit(limit: number, windowMs: number): RequestHandler {
  ...
  return (req, res, next) => {
    const now = Date.now(),
      key = res.locals.actor?.id || req.ip || "unknown";
    if (entries.size > 5000)
      for (const [id, entry] of entries)
        if (entry.until <= now) entries.delete(id);
    if (!entries.has(key) && entries.size >= 10000)
      return next(new HttpError(429, "Please try again later."));
    ...
    if (++entry.count > limit) {
      ...
      return next(
        new HttpError(429, "Too many attempts. Please try again later."),
      );
    }
    next();
  };
